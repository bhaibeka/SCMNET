...
{	
	...
	barcode <- do.call(rbind, lapply(edges, function(edge, nn, theweights) {
                        ...
                        c(i,j, dataset[,i] > dataset[,j])  ## select columns of dataset, j is an element of 1:nn
					}, nn=ncol(adjmatrix)))
    barcode <- barcode[barcode[ ,1] != barcode[ ,2], , drop=FALSE]  ## removes diagonal values gene1_gene1 gene1_gene1
	rownames(barcode) <- paste(rownames(adjmatrix)[barcode[ ,1]], rownames(adjmatrix)[barcode[ ,2]], sep="-")  ## rownames gene1_gene2
...
`compute.barcode.proba` <- function(dataset, theweight, adjmatrix)
{
    ...
    barcode <- barcode[barcode[ ,1] != barcode[ ,2], , drop=FALSE]  ## removes diagonal values gene1_gene1 gene1_gene1
	rownames(barcode) <- paste(rownames(adjmatrix)[barcode[ ,1]], rownames(adjmatrix)[barcode[ ,2]], sep="-")  ## rownames gene1_gene2
    # barcode <- barcode1[ ,-c(1,2), drop=FALSE]
	return(barcode)
}
...
`feature.selection` <- function(proba, barcode, n, top,redundant, w){
    ...
    salience <- lapply(1:ncol(subtype_freq), function(x){apply(abs(subtype_freq[,x] - subtype_freq[,-x]),1,min)})
        if(redundant == 1){
            # first genepair: MRMR depends highly on first gene pair
            gene_pair <- lapply(1:ncol(subtype_freq), function(x){sort(unlist(salience[[x]]), decreasing = TRUE)[1]})
            # features: make first gene pair first feature
            features[[1]] <- do.call(cbind, lapply(1:ncol(subtype_freq), function(x){names(gene_pair[[x]])}))
            i = 2
            while(i <= top){
                ...
                redundancy[[i-1]] <- do.call(cbind,lapply(1:ncol(subtype_freq), function(x){lapply(1:nrow(barcode), function(y){(vegdist(barcode[c(which(rownames(barcode) == names(gene_pair[[x]])),y),],methods = "jaccard"))})}))
                ...
                tmp3_redundancy <- lapply(1:ncol(subtype_freq),function(x){setNames(tmp2_redundancy[[x]], rownames(barcode))})
                # tmp4_redundancy: remove gene pairs in redundancy
                tmp4_redundancy <- lapply(1:ncol(subtype_freq), function(x){tmp3_redundancy[[x]][!names(tmp3_redundancy[[x]]) %in% lapply(features,`[[`,x)]})
                # tmp1_saliency: remove gene pairs in saliency
                tmp1_salience <- lapply(1:ncol(subtype_freq), function(x){salience[[x]][!names(salience[[x]]) %in% lapply(features,`[[`,x)]})
                # top_genes: saliency - redundancy
                top_genes <- lapply(1:ncol(subtype_freq), function(x){which.max(w*unlist(tmp1_salience[[x]])+unlist(tmp4_redundancy[[x]]))})
                ...
                features[[i]] <- do.call(cbind, lapply(1:ncol(subtype_freq), function(x){names(top_genes[[x]])}))
                gene_pair <- top_genes
                i = i + 1
            }
        ...
        features <- do.call(cbind,lapply(1:ncol(subtype_freq), function(x){names(sort(unlist(salience[[x]]), decreasing = TRUE))}[1:top]))
    }
    return(features)
}
...
`standard.vectors` <- function(proba, features, n,threshold){
    subtype_freq <- proba[,seq(n,ncol(proba),n)]
    # frequencies: finds the frequencies for each feature
    frequencies <- lapply(1:ncol(subtype_freq), function(x){subtype_freq[,x][rownames(subtype_freq) %in% features[,x]]})
    # standard_vector: if frequency is > 0.5 it is 1, else 0
    standard_vector <- lapply(1:ncol(subtype_freq), function(x){ifelse(frequencies[[x]] > 0.5, 1, 0)})
...
`jaccard.distance` <- function(proba, barcode, features,subtype.weights,n,standard){
    ...
    samples <-lapply(1:ncol(subtype_freq), function(x){barcode[rownames(barcode) %in% features[,x],]})
    ...
    tmp_samples <- lapply(1:ncol(subtype_freq),function(x){lapply(1:ncol(subtype_freq), function(y){cbind(standard[[x]],samples[[y]])})})
    ...
    jaccard <- lapply(1:ncol(subtype_freq),function(z){do.call(cbind,lapply(1:ncol(subtype_freq), function(x){lapply(1:ncol(tmp_samples[[z]][[x]]), function(y){vegdist(t(tmp_samples[[z]][[x]][,c(1,y)]),methods = "jaccard", binary = TRUE)})}))})
    # tmp1_jaccard: make jaccard matrix numeric
    tmp1_jaccard <- lapply(1:ncol(subtype_freq),function(x){apply(jaccard[[x]],2,as.numeric)})
    ...
    tmp2_jaccard <- lapply(1:ncol(subtype_freq),function(x){tmp1_jaccard[[x]][-1,]})
    #assign colnames to jaccard
    # colnames(tmp2_jaccard) <- c(1,2,3,4)
    #max_likelihood:  maximum likelihood
    max_likelihood <- do.call(cbind,lapply(1:ncol(subtype_freq),function(x){tmp2_jaccard[[x]][,x]/rowSums(tmp2_jaccard[[x]])}))
...
# feature selection
# features <- feature.selection(proba = subtype_barcode, barcode = binary_barcode, n = 3, top = 5, redundant = 0, w = 1.5)
# 
# # standard vectors
...
for(i in 1:nfold){
    ...
    features <- feature.selection(proba = subtype_barcode, barcode = train, n = 3, top = 1, redundant = 0, w = 1)
    standard <- standard.vectors(proba = subtype_barcode,features = features,n = 3, threshold = 0.5)
